...
import { changePasswordOldApi } from '../api'
...
const canSubmit = computed(() => {
  return oldPwd.value && newPwd.value && confirmPwd.value && pwdMatch.value
})
...
async function submit() {
  ...
  try {
    const res = await changePasswordOldApi({
  ...
  <div class="pwd-page">
    <div class="pwd-card">
      ...
      <div class="pwd-header">
        <h1 class="pwd-title">通过原密码修改</h1>
        <p class="pwd-desc">输入当前密码验证身份</p>
      </div>

      <!-- 原密码 -->
      <div class="field">
        <label class="field-label">原密码</label>
        <input v-model="oldPwd" type="password" placeholder="请输入当前密码" class="field-input" />
      </div>
